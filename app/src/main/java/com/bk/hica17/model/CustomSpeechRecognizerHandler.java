 ...
 */
public class CustomSpeechRecognizerHandler extends DbmHandler<Float> implements RecognitionListener {
     ...
     */
    public CustomSpeechRecognizerHandler(@NonNull Context context) {
     ...
     */
    public CustomSpeechRecognizerHandler(@NonNull Context context, float minRmsDbValue, float maxRmsDbValue) {
     ...
     */
    public CustomSpeechRecognizerHandler innerRecognitionListener(@Nullable RecognitionListener innerRecognitionListener) {
    ...
    protected void onDataReceivedImpl(Float rmsdB, int layersCount, float[] dBmArray, float[] ampsArray) {
        for (int i = 0; i < layersCount; i++) {
            dBmArray[i] = normalize(rmsdB, minRmsDbValue, maxRmsDbValue);
            ampsArray[i] = 1;
        }
    }
    ...
    public void onPartialResults(Bundle partialResults) {
        ...
        float factor = rand.nextInt((int)maxRmsDbValue);
        float rmsDb = 3.0f;
        if (matches !=  null && matches.size() > 0) {
            rmsDb = matches.size() * factor;
            Log.e("tuton", "rmsD:" + rmsDb);
        }
        onDataReceived(rmsDb);
        if (innerRecognitionListener != null) {
